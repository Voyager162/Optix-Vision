...
public class LedTriggers {
    ...
    public static void createLEDTriggers() {
        ...
        coralHasPieceTrigger.onTrue(Commands.runOnce(() -> Robot.led.setLEDStatusIndicator(StatusIndicator.PIECE)));

        Trigger scoringHasPieceTrigger = new Trigger(() -> Robot.scoringRoller.hasPiece());
        scoringHasPieceTrigger.onTrue(Commands.runOnce(() -> Robot.led.setLEDStatusIndicator(StatusIndicator.PIECE)));

        Trigger algaeKnockTrigger = new Trigger(() -> Robot.scoringRoller.getIsAlgaeMode());
        algaeKnockTrigger.onTrue(Commands.runOnce(() -> Robot.led.setLEDStatusIndicator(StatusIndicator.ALGAE)));
