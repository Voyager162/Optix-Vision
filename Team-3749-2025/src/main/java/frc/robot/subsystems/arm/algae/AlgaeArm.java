...
/**
 * Subsystem class for the arm. Handles control, logging, and state transitions
 * for the Algae Arm
 * subsystem.
 *
...
public class AlgaeArm extends Arm {
	...
	private PIDController controller = new PIDController(AlgaeConstants.kP, AlgaeConstants.kI, AlgaeConstants.kD);
	...
	private LoggedMechanismLigament2d armLigament = armRoot.append(new LoggedMechanismLigament2d("Algae Arm", 17, 0));

	/**
	 * Constructor for the AlgaeArm subsystem. Determines if simulation or real
	 * hardware is used.
	 */
	public AlgaeArm() {
	...
	private void runState() {
		switch (state) {
			case STOWED:
				setVoltage(
						controller.calculate(data.positionUnits, AlgaeConstants.stowSetPoint_rad)
								+ calculateFeedForward());
				break;
			case PROCESSOR:
				setVoltage(
						controller.calculate(data.positionUnits, AlgaeConstants.processorSetPoint_rad)
								+ calculateFeedForward());
				break;
			case ALGAE_PICKUP:
				setVoltage(
						controller.calculate(data.positionUnits, AlgaeConstants.algaePickUpSetPoint_rad)
								+ calculateFeedForward());
				break;
			case STOPPED:
