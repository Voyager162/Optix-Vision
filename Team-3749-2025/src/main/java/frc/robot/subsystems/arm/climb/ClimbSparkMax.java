...
/**
 * IO implementation for the climb subsystem's sparkmax I had to make two different motors because
 * we are using two motors on the climb I have decided to have one of the motors be the primary
 * motor taking care of all the data the secondary will just be there to provide more power to the
 * climb
 *
 * @author Elise Lin
...
public class ClimbSparkMax implements ArmIO {
  ...
  /**
   * Takes in the data from ArmData and uses it to update the data on the position of the motor,
  ...
  /**
   * Takes the volts parameter, then uses inputVolts to set the motor voltage Clamp takes the volts
   * and makes sure that the amount of volts isn't above or below the boundary for the voltage
