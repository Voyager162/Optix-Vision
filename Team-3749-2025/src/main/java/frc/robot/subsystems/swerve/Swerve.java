...
public class Swerve extends SubsystemBase {
	...
	private ShuffleData<Double[]> odometryLog = new ShuffleData<Double[]>(this.getName(), "odometry",
	...
	private ShuffleData<Double> rotationalVelocityLog = new ShuffleData<Double>(this.getName(), "rotational velocity",
			0.0);

	private ShuffleData<Boolean> gyroConnectedLog = new ShuffleData<Boolean>(this.getName(), "gyro connected", false);

	private ShuffleData<Boolean> gyroCalibratingLog = new ShuffleData<Boolean>(this.getName(), "gyro calibrating",
			false);

	private ShuffleData<Double> headingLog = new ShuffleData<Double>(this.getName(), "heading", 0.0);

	private ShuffleData<Boolean> utilizeVisionLog = new ShuffleData<Boolean>(this.getName(), "utilize vision", true);

	private ShuffleData<Double[]> setpointPositionLog = new ShuffleData<Double[]>(this.getName(), "setpoint position",
			new Double[] { 0.0, 0.0, 0.0 });
	private ShuffleData<Double> setpointVelocityLog = new ShuffleData<Double>(this.getName(), "setpoint velocity", 0.0);
	private ShuffleData<Double> setpointRotationalVelocityLog = new ShuffleData<Double>(this.getName(),
			"setpoint rotational velocity", 0.0);
	private ShuffleData<Double> setpointAccelerationLog = new ShuffleData<Double>(this.getName(),
			"setpoint acceleration", 0.0);

	private ShuffleData<Double> setpointRotationalAccelerationLog = new ShuffleData<Double>(this.getName(),
			"setpoint rotational acceleration", 0.0);
	...
	private void logData() {
		...
		double robotVelocity = Math.hypot(getChassisSpeeds().vxMetersPerSecond, getChassisSpeeds().vyMetersPerSecond);
