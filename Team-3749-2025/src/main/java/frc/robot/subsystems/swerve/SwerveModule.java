...
/**
 * General class for swerve modules that interacts with the interface. Handles
 * all logic relating to
 * individual modules
 *
 * @author Noah Simon
 * @author Rohin Sood
...
public class SwerveModule {
	...
	public SwerveModule(int index, SwerveModuleIO SwerveModule) {
		...
		drivingFeedFordward = new SimpleMotorFeedforward(
		...
		// Tab, name, data
		driveSpeed = new ShuffleData<>("Swerve/" + name, "drive speed", moduleData.driveVelocityMPerSec);
		drivePosition = new ShuffleData<>("Swerve/" + name, " drive position", moduleData.driveVelocityMPerSec);
		driveTemp = new ShuffleData<>("Swerve/" + name, "drive temp", moduleData.driveVelocityMPerSec);
		driveVolts = new ShuffleData<>("Swerve/" + name, "drive volts", moduleData.driveVelocityMPerSec);
		driveCurrent = new ShuffleData<>("Swerve/" + name, "drive current", moduleData.driveVelocityMPerSec);

		turningSpeed = new ShuffleData<>("Swerve/" + name, "turning speed", moduleData.driveVelocityMPerSec);
		turningPosition = new ShuffleData<>("Swerve/" + name, "turning position", moduleData.driveVelocityMPerSec);
		turningTemp = new ShuffleData<>("Swerve/" + name, "turning temp", moduleData.driveVelocityMPerSec);
		turningVolts = new ShuffleData<>("Swerve/" + name, "turning volts", moduleData.driveVelocityMPerSec);
		turningCurrent = new ShuffleData<>("Swerve/" + name, "turning current", moduleData.turnCurrentAmps);
	}
	...
	public void setTurnPosition(double positionRad) {
		double turning_volts = turningPidController.calculate(moduleData.turnAbsolutePositionRad, positionRad);
