...
/**
 * All setup and helper methods for auto routines, including the choreo factory and auto selector
 *
 * @author Noah Simon
 */
public class AutoUtils {
  ...
  private static void setupFactory(boolean isFlipped) {
    /**
     * Swerve Pose Supplier Reset Odometry Method Swerve Controller (PID and set chassis speeds)
     * Alliance Supplier for swapping Swerve Subsystem for scheduling Bindings, created above
     */
    ...
    factory =
        new AutoFactory(
            ...
            (Pose2d startingPose) ->
                Robot.swerve.setOdometry(isFlipped ? getXFlippedPose(startingPose) : startingPose),
  ...
  /** setup the choreo auto chooser and assign it to the Shuffleboard/Auto tab of networktables */
  ...
  public static Command getSingleTrajectory(String trajectoryName) {
    ...
    routine.active().onTrue(factory.resetOdometry(trajectoryName).andThen(trajectoryCommand));
  ...
  /**
   * Each pos will be flipped across the X-axis using Choreo's Flip Util Returns new pos Used in
   * auto factory for flipped paths
   */
  private static Pose2d getXFlippedPose(Pose2d pos) {
    ...
    return new Pose2d(newX, newY, new Rotation2d(newHeading));
  }
}
